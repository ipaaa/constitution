...
- **來歷**：feature `015 反方意見與中立呈現`（2026-04-30 完成，PR #14）。來源是 **0416 會議 46:12** 的決定 —— 面對進步派與藍白派學者立場分歧，需引入反方意見並以中立語氣呈現。該紀錄明載「**允中負責整理現有反方意見分析材料**」，但功能上線時材料尚未到位，先放了 AI 生成的範例內容，之後沒有替換
- **誰能做**：編輯台決定 → 工程執行
- **選項**：
...
**待決定**：`Track 2_opposing` 分頁要不要建。若不建，`opposing_views` 需要另一個歸屬（它目前直接寫在產物 `discussions.json` 裡，違反 design.md 不變式 #2「產物不得手改」）。

**這個決定會影響施工順序** —— `Track 2_opposing` 目前是唯一擋著工程施工的人工項目。若取消，工程可立即開始。
...
1. **決定反方意見怎麼辦** —— 一個決定同時解掉兩件事：
   - **P0-5**：`d1` 的虛構出處正在線上顯示，要拿掉假掛名、補真出處，還是先不顯示？
   - **P1-3**：`Track 2_opposing` 分頁要不要建？captain 表示從未想收集反方意見；0416 的原始決定是編輯加工、由允中整理，不是開分頁讓人填
   決定「不建分頁」的話，**工程的人工前置就全部清空，可以立刻開工**
