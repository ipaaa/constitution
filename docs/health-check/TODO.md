...
1. **建 `Track 2_opposing` 分頁**（design.md 項目 6）—— **人工，只有你能做，且是唯一還擋著工程的項目**
   實際只有 `d1` 一筆、2 個觀點，備份在 `docs/content-rescue/track2-rescue.md`。
   附帶待決：`d1` 的出處是佔位資料（`某學者` / `某大學法律系`），要補真實出處還是先不上線？
