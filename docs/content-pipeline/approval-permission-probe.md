...
**狀態**：record
**執行期間**：原始記錄只對四件事標日期，四件都是 2026-09-15——
P2 假通過的發現、P2 重驗、P6 跳過的決定、測試表保留的決定。
**其餘步驟的執行日期都沒有記錄，P7 也沒有。**
P7 的 `2026-09-07` 是 captain 核准把該項納入之日，不是執行日。
feature 044 的 implement stage 於 2026-09-08 起算。
...
> **本檔的記錄粒度有七個已知缺口，先講清楚：**
> 1. 各步驟的 UTC 時間沒有逐步記錄。有標日期的只有上面「執行期間」列出的那四件事。
...
> 6. **P5 有三欄未查核**：`review_decision` 與 `reject_reason` 的實際值，以及 `approved_fingerprint`。
>    `probe-reject` 是輸入值，不是觀察到的欄位值。見「P5」一節。
> 7. **指紋核對只比對開頭 8 碼與結尾 8 碼**，不是票內 AC-1 要求的 64 字元逐字比對。
>    見「指紋核對」一節。
>
> 這七項都是記錄缺口，不是行為失敗。各命題的行為結論不受影響，但**證據強度低於 AC 原文要求**。
> **本檔只記錄真正觀察到的事。沒有觀察到的，寫成缺口，不寫成事實。**
> **這份清單要與內文的缺口陳述數目一致。日後補記任何一項缺口，這裡要同步加。**
